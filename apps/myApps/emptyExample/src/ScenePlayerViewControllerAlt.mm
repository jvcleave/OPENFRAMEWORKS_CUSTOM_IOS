#import "ScenePlayerViewControllerAlt.h"
...
@implementation ScenePlayerViewControllerAlt
...
-(void)viewDidLoad
{
    ...
    self.glLayerView.multipleTouchEnabled = YES;
...
- (void)updateDimensions 
{
    ...
    self->window->windowSize.set(self.view.bounds.size.width * scaleFactor, self.view.bounds.size.height * scaleFactor, 0);
    ofLogVerbose() << "windowPos: " <<  self->window->windowPos;
    ofLogVerbose() << "windowSize: " <<  self->window->windowSize;
    ...
    ofLogVerbose() << "screenSize: " <<  self->window->screenSize;
